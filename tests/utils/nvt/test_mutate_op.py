...
def test_transform(df: DataFrameType):
    mutate_op = MutateOp(example_transform, output_columns=[('A_new', np.dtype('int64')), ('B_new', np.dtype('int64'))])
    col_selector = ColumnSelector(['A', 'B'])
    transformed_df = mutate_op.transform(col_selector, df)
...
def test_transform_lambda(df: DataFrameType):
    mutate_op = MutateOp(lambda col_selector,
...
def test_transform_additional_columns(df: DataFrameType):
    ...
    mutate_op = MutateOp(additional_transform,
                         output_columns=[('A_new', np.dtype('int64')), ('B_new', np.dtype('int64')),
                                         ('D', np.dtype('int64'))])
    col_selector = ColumnSelector(['A', 'B'])
    transformed_df = mutate_op.transform(col_selector, df)
...
def test_column_mapping():
    mutate_op = MutateOp(example_transform, output_columns=[('A_new', np.dtype('int64')), ('B_new', np.dtype('int64'))])
    col_selector = ColumnSelector(['A', 'B'])
    column_mapping = mutate_op.column_mapping(col_selector)
...
def test_compute_output_schema():
    ...
    output_schema = mutate_op.compute_output_schema(input_schema, col_selector)
